<template>
  <div class="unlock">
    ...
    <SettingView
      v-if="current && current[0] === 'setting'"
...
const getKeyRingStore = async () => {
  await keyRingStoreFunction.refreshKeyRingStatus();
  account.address = selectKeyInfo.value?.address;
  account.name = selectKeyInfo.value?.name;
  if (account.address) {
    queryBankBalance(account.address, "unyan").then((res) => {
      account.amount = Number(
        (Number(res.balance.amount) / 1000000).toFixed(2)
      );
    });
  }
};
